...
func testUnmarshal(t *testing.T, f func(s *UnmarshalState) any, data string, expected any) {
	t.Helper()
	if expectedValue := reflect.ValueOf(expected); expectedValue.Type().Kind() == reflect.Pointer {
		expected = expectedValue.Elem().Interface()
	}
	s := NewUnmarshalState([]byte(data), DefaultUnmarshalerConfig)
	actual := f(s)
	if actualValue := reflect.ValueOf(actual); actualValue.Type().Kind() == reflect.Pointer {
